...
const AppDetails = () => {
  ...
  useEffect(() => {
    fetch('/appData.json')
      ...
      .then((data) => {
        ...
        const isAlreadyInstalled = savedApps.some((saved) => saved.id == id);
  ...
  const handleInstall = () => {
    ...
    if (!savedApps.find(a => a.id === app.id)) {
      localStorage.setItem('installedApps', JSON.stringify([...savedApps, app]));
      setInstalled(true);
    ...
    <div className="w-11/12 mx-auto py-10 font-sans">
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        <div className="w-full md:w-150 md:h-80 shrink-0">
          <img src={app.image} alt={app.title} className="rounded-3xl shadow-md w-full h-full object-cover border border-gray-100" />
        </div>
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-6 mt-1">
            <div className="flex flex-col">
              ...
              <span className="text-gray-500 text-xs uppercase tracking-wider font-semibold">{app.reviews.toLocaleString()} Reviews</span>
            </div>
            <div className="w-px h-10 bg-gray-200 hidden sm:block"></div>
            <div className="flex flex-col">
              <span className="text-gray-900 font-bold text-lg">{(app.downloads / 1000000).toFixed(0)}M+</span>
      ...
      <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 shadow-sm mb-10">
